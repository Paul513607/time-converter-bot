...
class Converter:
    ...
    def parse_message(self, message: str, tz_string: str) -> int:
        ...
        # Match 'in X hours'
        if match := Converter.RELATIVE_FUTURE.search(message):
            hours = int(match.group(1))
            target_time = now + timedelta(hours=hours)
            return int(target_time.timestamp())
        
        # Match 'X hours ago'
        if match := Converter.RELATIVE_PAST.search(message):
            hours = int(match.group(1))
            target_time = now - timedelta(hours=hours)
            return int(target_time.timestamp())
